...
async function sendCustomerData() {
    ...
    let role = document.getElementById("in-famcount").value;
    ...
    const vendor = JSON.stringify(customerobj);
    console.log(vendor);
...
logbtn.addEventListener("click", async (e) => {
    ...
    let loginApi = await window.fetch(
        `http://localhost:8080/vendor?email=${email}&pwd=${psw}`
    );
    ...
    if (loginJson.data === "wrong email") {
        ...
        window.location.replace("http://127.0.0.1:5500/VendorHome.html");
        console.log("done");
    }
});
